# JavaScript ES11/ES12 Features - Mini Project

## Project Overview
Build a practical application that demonstrates your understanding of JavaScript ES11/ES12 Features.
...
1. Apply optional chaining (?.) in a practical context
2. Implement javascript es11/es12 features following best practices
...
- Implement Optional chaining (?.) and Nullish coalescing (??) and Promise.allSettled
...
```javascript
/**
 * JavaScriptES11ES12Features - JavaScript ES11/ES12 Features
 * A practical implementation demonstrating javascript es11/es12 features
 */
class JavaScriptES11ES12Features {
  constructor(options = {}) {
...
const project = new JavaScriptES11ES12Features();
try {
  project.add("Optional chaining (?.)");
  project.add("Nullish coalescing (??)");
...
1. Implement the optional chaining (?.) functionality
2. Add Nullish coalescing (??) capabilities
...
```
Processed: [
  { original: "Optional chaining (?.)", processed: "OPTIONAL CHAINING (?.)", timestamp: ... },
  { original: "Nullish coalescing (??)", processed: "NULLISH COALESCING (??)", timestamp: ... }
]
History: [
  { action: "add", item: "Optional chaining (?.)", timestamp: ... },
  { action: "add", item: "Nullish coalescing (??)", timestamp: ... }
]
```
