# JavaScript Cache API - Mini Project

## Project Overview
Build a practical application that demonstrates your understanding of JavaScript Cache API.
...
1. Apply cachestorage object in a practical context
2. Implement javascript cache api following best practices
...
- Implement CacheStorage object and caches.open() and Adding to cache (add, addAll, put)
...
```javascript
/**
 * JavaScriptCacheAPI - JavaScript Cache API
 * A practical implementation demonstrating javascript cache api
 */
class JavaScriptCacheAPI {
  constructor(options = {}) {
...
const project = new JavaScriptCacheAPI();
try {
  project.add("CacheStorage object");
  project.add("caches.open()");
...
1. Implement the cachestorage object functionality
2. Add caches.open() capabilities
...
```
Processed: [
  { original: "CacheStorage object", processed: "CACHESTORAGE OBJECT", timestamp: ... },
  { original: "caches.open()", processed: "CACHES.OPEN()", timestamp: ... }
]
History: [
  { action: "add", item: "CacheStorage object", timestamp: ... },
  { action: "add", item: "caches.open()", timestamp: ... }
]
```
