# JavaScript File Uploader - Mini Project

## Project Overview
Build a practical application that demonstrates your understanding of JavaScript File Uploader.
...
1. Apply drag and drop upload in a practical context
2. Implement javascript file uploader following best practices
...
- Implement Drag and drop upload and File validation (type, size) and Upload progress bar
...
```javascript
/**
 * JavaScriptFileUploader - JavaScript File Uploader
 * A practical implementation demonstrating javascript file uploader
 */
class JavaScriptFileUploader {
  constructor(options = {}) {
...
const project = new JavaScriptFileUploader();
try {
  project.add("Drag and drop upload");
  project.add("File validation (type, size)");
...
1. Implement the drag and drop upload functionality
2. Add File validation (type, size) capabilities
...
```
Processed: [
  { original: "Drag and drop upload", processed: "DRAG AND DROP UPLOAD", timestamp: ... },
  { original: "File validation (type, size)", processed: "FILE VALIDATION (TYPE, SIZE)", timestamp: ... }
]
History: [
  { action: "add", item: "Drag and drop upload", timestamp: ... },
  { action: "add", item: "File validation (type, size)", timestamp: ... }
]
```
