# JavaScript Browser History API - Mini Project

## Project Overview
Build a practical application that demonstrates your understanding of JavaScript Browser History API.
...
1. Apply history.pushstate in a practical context
2. Implement javascript browser history api following best practices
...
- Implement history.pushState and history.replaceState and popstate event
...
```javascript
/**
 * JavaScriptBrowserHistoryAPI - JavaScript Browser History API
 * A practical implementation demonstrating javascript browser history api
 */
class JavaScriptBrowserHistoryAPI {
  constructor(options = {}) {
...
const project = new JavaScriptBrowserHistoryAPI();
try {
  project.add("history.pushState");
  project.add("history.replaceState");
...
1. Implement the history.pushstate functionality
2. Add history.replaceState capabilities
...
```
Processed: [
  { original: "history.pushState", processed: "HISTORY.PUSHSTATE", timestamp: ... },
  { original: "history.replaceState", processed: "HISTORY.REPLACESTATE", timestamp: ... }
]
History: [
  { action: "add", item: "history.pushState", timestamp: ... },
  { action: "add", item: "history.replaceState", timestamp: ... }
]
```
