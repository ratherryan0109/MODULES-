# JavaScript Server-Sent Events - Mini Project

## Project Overview
Build a practical application that demonstrates your understanding of JavaScript Server-Sent Events.
...
1. Apply eventsource api in a practical context
2. Implement javascript server-sent events following best practices
...
- Implement EventSource API and Server-sent event format and Automatic reconnection
...
```javascript
/**
 * JavaScriptServerSentEvents - JavaScript Server-Sent Events
 * A practical implementation demonstrating javascript server-sent events
 */
class JavaScriptServerSentEvents {
  constructor(options = {}) {
...
const project = new JavaScriptServerSentEvents();
try {
  project.add("EventSource API");
  project.add("Server-sent event format");
...
1. Implement the eventsource api functionality
2. Add Server-sent event format capabilities
...
```
Processed: [
  { original: "EventSource API", processed: "EVENTSOURCE API", timestamp: ... },
  { original: "Server-sent event format", processed: "SERVER-SENT EVENT FORMAT", timestamp: ... }
]
History: [
  { action: "add", item: "EventSource API", timestamp: ... },
  { action: "add", item: "Server-sent event format", timestamp: ... }
]
```
