# JavaScript Event Loop - Mini Project

## Project Overview
Build a practical application that demonstrates your understanding of JavaScript Event Loop.
...
1. Apply call stack in a practical context
2. Implement javascript event loop following best practices
...
- Implement Call stack and Task queue (macrotasks) and Microtask queue
...
```javascript
/**
 * JavaScriptEventLoop - JavaScript Event Loop
 * A practical implementation demonstrating javascript event loop
 */
...
const project = new JavaScriptEventLoop();
try {
  project.add("Call stack");
  project.add("Task queue (macrotasks)");
...
1. Implement the call stack functionality
2. Add Task queue (macrotasks) capabilities
...
```
Processed: [
  { original: "Call stack", processed: "CALL STACK", timestamp: ... },
  { original: "Task queue (macrotasks)", processed: "TASK QUEUE (MACROTASKS)", timestamp: ... }
]
History: [
  { action: "add", item: "Call stack", timestamp: ... },
  { action: "add", item: "Task queue (macrotasks)", timestamp: ... }
]
```
